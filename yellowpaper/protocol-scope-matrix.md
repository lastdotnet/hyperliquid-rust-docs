...
- `claim.action_delayer_maturity_surface` - ActionDelayer maturity is queue-entry driven, while mode semantics remain partly open (`execution`, `implemented`, `active`)
- `claim.begin_block_ordering_surface` - Begin-block ordering is partly confirmed, but BOLE and delayed-action placement still need closure (`execution`, `confirmed`, `open`)
...
- `claim.begin_block_bole_ordering_conflict` - Local begin-block notes disagree on where BOLE liquidations run (`execution`, `observed`, `open`)
